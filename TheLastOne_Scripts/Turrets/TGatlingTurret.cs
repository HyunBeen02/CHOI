...
public class TGatlingTurret : Turret, ITurretFunction
{
    ...
    public void attackToTarget()
    {
        ...
        for (int idx = 0; idx < turretProjectileGenerator.TBullets.Length; idx++)
        {
            if (!turretProjectileGenerator.TBullets[idx].activeSelf)
            {
                ...
                turretProjectileGenerator.TBullets[idx].transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles.x, transform.rotation.eulerAngles.y - 90 + ranY, transform.rotation.eulerAngles.z);
